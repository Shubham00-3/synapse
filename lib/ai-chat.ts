...
export function buildKnowledgeContext(items: Item[], maxItems = 10): string {
  ...
  const itemsContext = items.slice(0, maxItems).map((item, index) => {
    ...
    // For YouTube videos, show description
    const contentLabel = item.type === 'youtube' ? 'Video Description' : 'Content';
    
    // Show content preview
    const contentPreview = item.content ? item.content.slice(0, 300) : 'No content';
...
- For YouTube videos, work with the video description and AI-generated summaries
